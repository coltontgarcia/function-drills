...
function nameCheck(name) {
  if (name === "Colton") {
    return "How are you Colton?"
  } else if (name === "Rayne") {
    return "how are you Rayne?"
  } else {
    return `cool name, ${name}`
  }
}

let nameGreeting = nameCheck("Colton")
...
function faveColorFinder(color) {
  if (color === "red") {
    ...
    return "work on your color choice"
  }
}
...
function thatsOdd(num) {
  if (num % 2 === 0) {
    return "that is not an odd number!"
  } else {
...
let totalFrogs = buyingFrogs(100)
...
let arrayAscends = ArrayAscending(sampleArray)
